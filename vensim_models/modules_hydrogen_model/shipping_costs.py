...
def be_ship_cost():
    return (
        ...
        * (electricity_price() * 1000 + charging_infrastructure_cost() * usd_to_eur())
    )
...
def fc_ship_cost():
    return (
        ...
        * (opex_electric_ship() + ship_engine_af())
        + yearly_h2_consumption() * green_h2_cost() / 33.33 * 1000
    )
...
def meoh_containership_cost():
    return (
        ...
        + (green_meoh_av_cost() * 3600) * yearly_containership_consumption() / 1000
    )
...
def meoh_ship_cost():
    return (
        ...
        * auxilliary_battery_capacity()
        * battery_cost()
    ) * hfo_vs_meoh_capex() + yearly_hfo_consumption() * (green_meoh_av_cost() * 3600)
...
def nh3_containership_cost():
    return (
        ...
        + (green_nh3_cost() * 3600) * yearly_containership_consumption() / 1000
    )
