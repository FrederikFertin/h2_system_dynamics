"""
Module methanol_production__fossil_meoh_missing
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "blue_biomeoh_price": 1,
        "green_biomeoh_price": 3,
        "grey_biomeoh_price": 1,
        "green_emeoh_price": 1,
    },
)
def biomeoh_competitiveness():
    return np.minimum(
        np.minimum(
            blue_biomeoh_price() / green_biomeoh_price(),
            grey_biomeoh_price() / green_biomeoh_price(),
        ),
        green_emeoh_price() / green_biomeoh_price(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"biomeoh_competitiveness": 1},
)
def biomeoh_inno_switch():
    return if_then_else(biomeoh_competitiveness() > 0.5, lambda: 1, lambda: 0)
...
def blue_meoh_competitiveness():
    return np.minimum(
        np.minimum(
            grey_biomeoh_price() / blue_biomeoh_price(),
            green_biomeoh_price() / blue_biomeoh_price(),
        ),
        green_emeoh_price() / blue_biomeoh_price(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"blue_meoh_competitiveness": 1},
)
def blue_meoh_inno_switch():
    return if_then_else(blue_meoh_competitiveness() > 0.5, lambda: 1, lambda: 0)
...
def emeoh_competitiveness():
    return np.minimum(
        np.minimum(
            blue_biomeoh_price() / green_emeoh_price(),
            grey_biomeoh_price() / green_emeoh_price(),
        ),
        green_biomeoh_price() / green_emeoh_price(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"emeoh_competitiveness": 1},
)
def emeoh_inno_switch():
    return if_then_else(emeoh_competitiveness() > 0.5, lambda: 1, lambda: 0)
...
@component.add(
    ...
    other_deps={
        "_integ_errorint_meoh": {
            "initial": {"methanol_demand": 1},
            "step": {"k_i": 1, "error_meoh": 1},
        }
    },
)
def errorint_meoh():
...
_integ_errorint_meoh = Integ(
    lambda: k_i() * error_meoh(),
    lambda: methanol_demand() * 0.0179,
    "_integ_errorint_meoh",
)
...
def grey_meoh_competitiveness():
    return np.minimum(
        np.minimum(
            blue_biomeoh_price() / grey_biomeoh_price(),
            green_biomeoh_price() / grey_biomeoh_price(),
        ),
        green_emeoh_price() / grey_biomeoh_price(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"biomeoh": 1, "meoh_biomass_usage": 1},
)
def meoh_biomass_demand():
    return biomeoh() * meoh_biomass_usage() * 10**6
...
_integ_meoh_reinvestment = Integ(
    ...
    lambda: methanol_demand() * (0.0179 + 1 / meoh_plant_lifetime()),
    "_integ_meoh_reinvestment",
)


@component.add(
    name="methanol demand",
    units="MT MeOH",
    comp_type="Auxiliary",
...
def methanol_demand():
    """
    European MeOH demand. Based on 2023 level of 11.3 MT. Forecasted 3.96% CAGR from 2023 to 2034. Assumed 3% CAGR from 2034 to 2050. Extrapolated back to 2019 with same growth as forecast (3.96%). Source: https://www.chemanalyst.com/industry-report/europe-methanol-market-215
    """
    return np.interp(
        time(),
        [
