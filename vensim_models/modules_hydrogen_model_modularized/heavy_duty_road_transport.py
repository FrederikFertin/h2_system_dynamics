...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "hd_ownership_cost_diesel": 1,
        "hd_ownership_cost_bev": 2,
        "hd_ownership_cost_fcev": 1,
    },
)
def hd_bev_competitiveness():
    return np.minimum(
        hd_ownership_cost_diesel() / hd_ownership_cost_bev(),
        hd_ownership_cost_fcev() / hd_ownership_cost_bev(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"hd_bev_competitiveness": 1},
)
def hd_bev_inno_switch():
    return if_then_else(hd_bev_competitiveness() > 0.5, lambda: 1, lambda: 0)
...
def hd_bev_level():
    return (
        1
        / (1 + np.exp(slope() * (cross_innovation() - hd_bev_competitiveness())))
        * hd_bev_consumption()
        / sum_hd_rt()
    )
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "hd_ownership_cost_bev": 1,
        "hd_ownership_cost_fcev": 2,
        "hd_ownership_cost_diesel": 1,
    },
)
def hd_fcev_competitiveness():
    return np.minimum(
        hd_ownership_cost_bev() / hd_ownership_cost_fcev(),
        hd_ownership_cost_diesel() / hd_ownership_cost_fcev(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"hd_fcev_competitiveness": 1},
)
def hd_fcev_inno_switch():
    return if_then_else(hd_fcev_competitiveness() > 0.5, lambda: 1, lambda: 0)
...
def hd_fcev_level():
    return (
        1
        / (1 + np.exp(slope() * (cross_innovation() - hd_fcev_competitiveness())))
        * hd_fcev_consumption()
        / sum_hd_rt()
    )
...
@component.add(
    ...
    depends_on={
        "hd_ownership_cost_bev": 1,
        "hd_ownership_cost_diesel": 2,
        "hd_ownership_cost_fcev": 1,
    },
)
def hd_fossil_competitiveness():
    return np.minimum(
        hd_ownership_cost_bev() / hd_ownership_cost_diesel(),
        hd_ownership_cost_fcev() / hd_ownership_cost_diesel(),
    )
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"hd_fossil_consumption": 1, "truck_lifetime": 1},
)
def hd_fossil_decay():
    return hd_fossil_consumption() / truck_lifetime()
...
def hd_fossil_level():
    return (
        1
        / (1 + np.exp(slope() * (cross_conventional() - hd_fossil_competitiveness())))
...
_integ_hd_rt_reinvestment = Integ(
    ...
    - hd_fcev_investment()
    - hd_bev_investment(),
    lambda: hd_rt_consumption() * (0.0179 + 1 / truck_lifetime()),
    "_integ_hd_rt_reinvestment",
)
...
@component.add(name="truck lifetime", comp_type="Constant", comp_subtype="Normal")
def truck_lifetime():
    return 17.5
