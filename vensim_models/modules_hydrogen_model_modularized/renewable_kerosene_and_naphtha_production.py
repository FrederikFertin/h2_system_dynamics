...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"hvo_jet_total_costs": 1, "jetfuel_fraction_bio": 1},
)
def biokero_cost():
    """
    5% of the cost is removed through coproduct selling of naphtha.
    """
    return hvo_jet_total_costs() / jetfuel_fraction_bio() * 0.85
...
def hvo_jet_total_costs():
    return (
        (
            biokero_electricity_usage() * electricity_price() * 1000
            + biokero_gas_usage() * blue_ng_cost() * 3.6
            + biokero_biomass_usage() * oil_biomass_price() * 3.6
            + biokero_h2_usage() * green_h2_price() / lhv_h2() * 1000
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"green_h2_price": 1, "lhv_h2": 1},
)
def hydrogen_cost():
    return green_h2_price() / (lhv_h2() * 3.6)


@component.add(
    name="Jetfuel fraction",
    units="percent",
    comp_type="Constant",
    comp_subtype="Normal",
)
def jetfuel_fraction():
    """
...
@component.add(
    name="Jetfuel fraction bio",
    units="percent",
    comp_type="Constant",
    comp_subtype="Normal",
)
def jetfuel_fraction_bio():
    """
    Percent of liquid fuels which is jetfuel
    """
    return 0.66
...
def synkero_cost():
    ...
    return (
        (
            ps_cc_cost() * synkero_co2_usage() / jetfuel_lhv() * 3.6
            + (
                1000 * electricity_price() * synkero_electricity_usage()
                + 1000 * green_h2_price() / lhv_h2() * synkero_h2_usage()
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"synkero_cost": 1, "jetfuel_fraction": 2},
)
def synnaphtha_cost():
    """
    ...
    """
    return (
        (synkero_cost() / 0.8 * jetfuel_fraction())
        * 0.2
        / (1 - jetfuel_fraction())
        * 1000
    )
