...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"ctrl_shipping": 1, "meoh_innovators": 1, "nh3_innovators": 1},
)
def demand_change_shipping():
    return ctrl_shipping() - meoh_innovators() - nh3_innovators()
...
@component.add(
    ...
    other_deps={
        "_integ_error_int_shipping": {
            "initial": {"international_shipping_consumption": 1},
...
_integ_error_int_shipping = Integ(
    ...
    lambda: international_shipping_consumption() / 40,
    "_integ_error_int_shipping",
)
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"international_shipping_consumption": 1, "sum_int_shipping": 1},
)
def error_shipping():
    return international_shipping_consumption() - sum_int_shipping()
...
_integ_hfo_shipping_consumption = Integ(
    ...
    lambda: international_shipping_consumption(),
    "_integ_hfo_shipping_consumption",
)
...
@component.add(
    ...
    comp_type="Lookup",
    comp_subtype="Normal",
    depends_on={"__lookup__": "_hardcodedlookup_int_shipping_consumption_forecast"},
)
def int_shipping_consumption_forecast(x, final_subs=None):
    """
    ...
    """
    return _hardcodedlookup_int_shipping_consumption_forecast(x, final_subs)


_hardcodedlookup_int_shipping_consumption_forecast = HardcodedLookups(
    [
        1990,
        1991,
        1992,
    ...
    "interpolate",
    {},
    "_hardcodedlookup_int_shipping_consumption_forecast",
)
...
@component.add(
    name="international shipping consumption",
    units="GWh",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def international_shipping_consumption():
    ...
    Considering an annual growth of 1,79%/year linearly compared to year 2019. Consumption is amount of fuel bunkered. Pre efficiency losses.
    """
    return fuel_use_index() * int_shipping_consumption_forecast(time())
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"nh3_shipping_consumption": 1, "nh3_lhv": 1, "nh3_h2_usage": 1},
)
def international_shipping_nh3_hydrogen_demand():
    """
    Convert from GWh to GJ, then from GJ to tons fuel, then from tons fuel to tons H2. Example 1: MeOH cons. [GWh] * 3600 [GJ/GWh] / 19.9 [GJ/t] / 15.7 [t MeOH/t H2] Example 2: NH3 cons. [GWh] * 3600 [GJ/GWh] / 18.6 [GJ/t] / 5.56 [t NH3/t H2]
    """
    return nh3_shipping_consumption() * 3600 / nh3_lhv() / nh3_h2_usage()
...
@component.add(
    ...
    other_deps={
        "_integ_shipping_reinvestment": {
            "initial": {
                "initial_time": 2,
                "int_shipping_consumption_forecast": 2,
                "ship_lifetime": 1,
            },
            "step": {
...
_integ_shipping_reinvestment = Integ(
    ...
    - meoh_investment()
    - nh3_investment(),
    lambda: int_shipping_consumption_forecast(initial_time())
    * (1 + 1 / ship_lifetime())
    - int_shipping_consumption_forecast(initial_time() - 1),
    "_integ_shipping_reinvestment",
)
